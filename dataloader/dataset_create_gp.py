    ...
    for scenario_name in SCENARIO_RANGE:
        ...
        for recording in tqdm(dataloader.test_data(),
            ...
            if recording_type == RecordingType.NORMAL_AND_ATTACK or recording_type == RecordingType.ATTACK:
                print(f'Name {recording.name}  Attack file')
            #     continue
            # if recording_type != RecordingType.NORMAL_AND_ATTACK and recording_type != RecordingType.ATTACK:
            #     print(f'Name {recording.name} {recording_type.name} is not Attack file, Skip')
            #     continue
            ...
            recoding_path = os.path.join(DATAOUT_DIR, scenario_name, TRAINING, recording.name)
            ...
            for syscall in recording.syscalls():
                ...
                if syscall_time - df.at[0, 'time'] > 3000000000:
                    ...
                    start_index = len(df[df['time'] < df.at[0, 'time'] + 200000000])
                    for i in range(start_index):
                        src_node = tuple([df.at[i, 'syscallInt'], df.at[i, 'Ret']])
                        tmp = df[i:][['syscallInt', 'Ret']][df[i:]['TID'].isin([df.at[i, 'TID']])]
                        if len(tmp) > 1:
                            tar_node = tuple([tmp.at[tmp.index[1], 'syscallInt'], tmp.at[tmp.index[1], 'Ret']])
                            scg.remove_one_edge(df.at[i, 'ProcessName'], src_node, tar_node)
                        else:
                            scg.remove_empty_node(df.at[i, 'ProcessName'], src_node)
                            print(f'{syscall_time} has the last tid {tid}')

                    # src_node = df.at[0, 'syscallInt']
                    # tmp = df[['syscallInt']][df['TID'].isin([df.at[0, 'TID']])]
                    # if len(tmp) > 1:
                    #     tar_node = tmp.at[tmp.index[1], 'syscallInt']
                    ...
                    df.drop(df.index[[i for i in range(start_index)]],  inplace=True)
